...
같은 회사 동료가 **각자 장바구니·구매자 정보**로 메뉴를 모으고, **배달은 한 건**으로 처리합니다.

1. 호스트가 모임 생성 → 링크/코드 공유  
2. 각자 메뉴 담기 → **내 메뉴 등록** (이 단계에선 어드민 미전달, `draft`)  
3. 호스트가 **「합배송 주문 보내기」** → 어드민에 접수 (`submitted`)  
4. Admin은 **접수된 합배송만** 처리 (모집 중 목록은 참고용)  
